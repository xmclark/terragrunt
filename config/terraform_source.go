...
func (src *TerraformSource) ReadVersionFile() (string, error) {
	return util.ReadFileAsString(src.VersionFile)
}

// Write a file into the DownloadDir that contains the version number of this source code. The version number is
// calculated using the EncodeSourceVersion method.
func (src *TerraformSource) WriteVersionFile() error {
	version := EncodeSourceVersion(src.CanonicalSourceURL)
	return errors.WithStackTrace(ioutil.WriteFile(src.VersionFile, []byte(version), 0640))
}
